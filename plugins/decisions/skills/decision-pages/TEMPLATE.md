  ...
  <head>
    ...
    <style>
      ...
      * {
      ...
      .option.selected .section-label::after {
        content: " · selected";
        color: var(--text-info);
      }
      .option h2 {
    ...
    <main>
      ...
      <script>
        (() => {
          ...
          if (!options.length || !title || !body) return;
          const sync = () => {
            const picked = options
              .filter((option) => option.classList.contains("selected"))
              .map((option) => option.querySelector("h2")?.textContent.trim())
              .filter(Boolean);
            if (!picked.length) return;
            title.value = `Approve: ${picked.join(", ")}`.slice(0, 120);
            body.value = `Approved options: ${picked.join("; ")}. Turn these into plans.`;
          };
          options.forEach((option) => {
            ...
            const toggle = () => {
              ...
              option.setAttribute(
                "aria-pressed",
                String(option.classList.contains("selected"))
              );
              sync();
            };
